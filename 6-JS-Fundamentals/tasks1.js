...
function greaterNumber(x, y) {
  const message = "Variables are out of scope";
  if (isNaN(x) || isNaN(y)) {
    return message;
  }
...
const array3 = [40, 18, 15, 2];
if (array3.length != 3) {
  throw new Error("Array must be of 3 elements");
}
let sum3 = array3.reduce(function (x, y) {
  return x + y;
}, 0);
...
//3ta tochka
const arrayReverse = [1, 15, 26, 45];
console.log(arrayReverse.toString().split("").reverse().join(""));
...
function getGCD(number1, number2) {
  const message = "You must put 2 numbers";

  if (typeof number1 !== "number" || typeof number2 !== "number") {
    // proverqva dali sa podadeni chisla
    return message;
  }

  if (arguments.length != 2) {
    //proverqva dali sa podadeni samo 2 chisla
    return message;
  }
...
let keysStudent = Object.keys(student10);
...
console.log(remove1st("tekst s mnogo povtoreniq v teksta", "tekst s"));
...
function lastWord(word, string) {
  let index = word.length - string.length;
...
let keys = Object.keys(student16).length;
console.log(keys);
